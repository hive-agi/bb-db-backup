#!/bin/bash
# Periodic Chroma vector database backup
# Copies SQLite from Docker container, retains last N days
# Skips if latest backup is less than MIN_AGE_HOURS old
#
# Requirements:
#   - Docker with running Chroma container
#
# Environment variables:
#   BACKUP_DIR          — backup directory (default: ~/backups/chroma)
#   CHROMA_CONTAINER    — container name (default: chroma)
#   CHROMA_DB_PATH      — path inside container (default: /data/chroma.sqlite3)
...
CHROMA_DB_PATH="${CHROMA_DB_PATH:-/data/chroma.sqlite3}"
