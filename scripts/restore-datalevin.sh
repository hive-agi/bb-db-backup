#!/bin/bash
# Restore Datalevin (LMDB) from tar.gz backup
# Usage: ./scripts/restore-datalevin.sh <backup_file.tar.gz> [target_dir]
#
# Environment variables:
#   DATALEVIN_DIR — target restore directory (or pass as 2nd argument)

set -e

BACKUP_FILE="${1:?Usage: $0 <backup_file.tar.gz> [target_dir]}"

# Resolve target directory
if [[ -n "$2" ]]; then
    DATALEVIN_DIR="$2"
elif [[ -n "$DATALEVIN_DIR" ]]; then
    : # already set
else
